...
cd train && air run --file train.yaml --watch
...
- The runner notebook loads `train/utils.py` via `%run ./utils` and the load-test notebook via `%run ../train/utils`, **not** an import. Its helpers (`load_yaml_config`, `config_str`/`config_int`/`config_float`/`config_bool`, `quote_identifier`, `full_name`, `get_spark_session`, plus `load_training_config`/`init_training_workspace` for the training context) become notebook-session globals. Do not add `from utils import ...` in notebooks (the setup notebook's guarded import exists only for local script runs).
- Training config flows through `utils.load_training_config()`, which returns a flat dict that the runner notebook and `train.py` each bind with `globals().update(...)` — so both see identical constants (`MODEL_NAME`, `TRAINING_OUTPUT_DIR`, `sft_table_q`, ...). It is deliberately a function, not top-level utils code: utils' other consumers (load-test/setup notebooks) resolve `train.yaml` relative to *their* directories and must not trigger schema/Spark side effects.
- Notebooks support dual execution: inside Databricks (global `spark`, `dbutils`) or locally via Databricks Connect. `get_notebook_dir()` and `get_spark_session()` in `utils.py` handle both paths; `setup/01_load_tabformer_dataset.py` gets the same helpers via `%run ../train/utils` in workspace runs, with a guarded `from utils import ...` fallback for local script execution.
...
- **Config coupling**: `catalog`, `schema`, and `sft_table` must agree across all three YAML files (the training values sit under `train.yaml`'s `training_config` key); `endpoint_name` must match between `train.yaml` and `serving_load_test.yaml`; `setup.yaml`'s `table` must equal `train.yaml`'s `source_table`.

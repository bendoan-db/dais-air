...
# MAGIC
# MAGIC This project-local deployment step takes a training run produced by `01_runner.py` (or the AIR CLI), merges the run's QLoRA (PEFT) adapter into the base model, registers the merged model to Unity Catalog as a custom LLM, and creates or updates a Mosaic AI Model Serving endpoint for it.
# MAGIC
...
# MAGIC
# MAGIC The adapter was trained on a 4-bit (QLoRA) base, but the merge here loads the base in bf16 via `peft.AutoPeftModelForCausalLM` and calls `merge_and_unload()`, so the packaged weights are full-precision bf16 that vLLM can serve directly.
# MAGIC
...
def register_custom_llm_model(adapter_output_dir: str, run_name: str):
    ...
    try:
        ...
        # load_model_for_merge loads the base in bf16 via
        # peft.AutoPeftModelForCausalLM; merge_and_unload folds the QLoRA
        # adapter into the base and returns a plain (non-PEFT) model.

...
# DBTITLE 1,Register and deploy the full-weight Qwen3 4B model
# MAGIC %md
# MAGIC # Register and deploy the full-weight Qwen3 4B model
# MAGIC
...
# MAGIC
# MAGIC The registered model starts the project-pinned vLLM OpenAI-compatible
# MAGIC server. Qwen3 is supported by this repository's FIPS-safe vLLM 0.11.2
# MAGIC and Transformers 4.57.6 serving stack.
# MAGIC
...
# MAGIC starts vLLM from the artifact root on port 8080.
...
def register_custom_llm_model(model_output_dir: str, run_name: str) -> dict:
    ...
    architectures = json.loads(
        (Path(model_output_dir) / "config.json").read_text(encoding="utf-8")
    ).get("architectures", [])
    if "Qwen3ForCausalLM" not in architectures:
        raise ValueError(
            f"Expected Qwen3ForCausalLM checkpoint architecture, got {architectures}"
        )
    print(f"Checkpoint architecture: {architectures}")
